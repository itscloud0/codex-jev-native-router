...
If built-in Image Gen returns an immediate empty-body HTTP 404, changing Jev's model/effort policy is not a demonstrated fix. On one affected Mac, the same failure occurred with concrete Sol and Astra as well as Jev Auto; a [Codex issue from before this router was installed](https://github.com/openai/codex/issues/43250) describes the identical error on another account. The tool exposes no failing URL or request ID, so its precise backend cause remains unknown. This router implements no image endpoint and never forwards its TypeSafe key to the image tool. A router-disabled A/B test would still be needed to exclude an indirect interaction with the global Codex base-URL setting. Test the built-in tool with a concrete model and a minimal prompt, then check that issue or report the result to Codex support. The optional OpenAI Image API is a separately billed path and is not enabled by this router.
