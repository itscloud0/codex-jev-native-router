...
`large_context_sol_floor_tokens` defaults to 48,000. At or above this measured context size, Auto and Shadow cannot downgrade below Sol, because a large active conversation is expensive to rebuild on a colder, weaker model. Set it to `0` to disable. This is a conservative guard, not a measured optimum. A local 96,000 setting can be tried to allow more Terra/Luna decisions in medium-length tasks while retaining the Sol floor for contexts around 100,000 tokens; measure quality and cache behavior before adopting it as a general default. Start a fresh task after a handoff when the work genuinely changes.
