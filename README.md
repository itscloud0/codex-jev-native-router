...
With `completion_v3`, Auto also remembers only the previous work-shape category and native route for one hour. A bare "continue" or "продолжай" in the same Desktop task can reuse that route without another Jev call if the model remains allowed, the measured context still permits it, and no concrete-model switch or effort override occurred. New or mixed instructions use the normal routing policy. No task text, source, or tool output is stored in the route lease. This saves a Jev call and preserves model affinity; it does not establish Pro allowance savings. `jev-codex report` counts `continuation_lease` decisions under `routes.reasons` and v3 work-shape categories under `routes.by_policy.completion_v3.work_shapes`, so the `unknown` rate is visible.
...
`large_context_sol_floor_tokens` defaults to 48,000. At or above this measured context size, Auto and Shadow cannot downgrade below Sol, because a large active conversation is expensive to rebuild on a colder, weaker model. Set it to `0` to disable. This is a conservative guard, not a measured optimum. On one Mac, a trial at 96,000 did not establish a benefit: Jev still proposed Sol for the few eligible turns, while privacy fallbacks and longer contexts also stayed on Sol. That installation returned to 48,000. Measure task outcomes and cache behavior before relaxing the guard. Start a fresh task after a handoff when the work genuinely changes.
...
`auto_policy` and `shadow_policy` each accept `baseline`, `completion_v1`, `completion_v2`, or `completion_v3`. Auto now defaults to `completion_v3`: one TypeSafe call with up to two independent Choices, work shape and reasoning depth. Deterministic code maps work shape to available model roles, applies the risk/context floor and allowlist, and sends an explicit `unknown` shape to Sol. `xhigh` on Luna/Terra upgrades to Sol; an explicit CLI effort override remains binding. Shadow still executes Sol and records only a proposal, regardless of policy. `completion_v2` remains available as the earlier joint Choice over supported model/effort pairs and is the installed Shadow comparison policy. `completion_v1` asks Jev to account for retries, corrections, context rebuilding, model capability, and measured prior-turn cached-input percentage where available. On a 48-case privacy-bounded historical replay, it proposed Sol on 26 cases versus 4 for `baseline`; those cases have no trustworthy outcome labels. The older joint policy corrected an observed bad Luna/Max route on a 100k-context live-operations task, with a deterministic Sol floor for large context. The new v3 policy was promoted after a seven-case synthetic smoke showed plausible roles and avoided v2's Luna/low answer for an evidence-poor "Continue the task" request. This is a safety/consistency result, **not** proof of quality-equivalent savings. All policies keep Jev's input bounded and omit source/tool output. These policies adapt ideas from [0xNatoshi/jev-codex-router](https://github.com/0xNatoshi/jev-codex-router) and [auto-codex-router](https://github.com/zhangqiang8vipp/auto-codex-router) under their MIT licenses; no source module was copied.

`unknown` means the bounded current-turn text does not establish the work shape. It is expected for short references to earlier turns and requests whose substance is in an image or omitted code; Jev does not see full history or attachment contents. When the allowlist and deterministic floor leave only Sol eligible, v3 no longer asks Jev to classify work shape. It asks only for effort and records `sole_eligible_model` under `routes.model_bases`; a Jev `low` effort is conservatively raised to `medium` when work shape was not classified, and the existing context/risk floor still applies. Older `unknown` records remain historical evidence. This does not make a long session cheaper in Pro units by itself.
...
For `completion_v2` and `completion_v3`, each fresh Jev Choice yields a privacy-safe decision receipt. V2 records the selected model/effort pair's probability and confidence; v3 records work shape, its probability/confidence, and effort confidence. Both record the resolved Jev model version. `jev-codex trace THREAD_UUID` shows these fields beside the route; `jev-codex report` shows confidence sample count and mean by policy. A cached decision retains its original receipt. Missing or malformed scores are recorded as null and never alter the selected executor. Per [TypeSafe's confidence guidance](https://docs.typesafe.ai/confidence), confidence is **not** a correctness probability or a calibrated routing threshold: it measures concentration of Jev's offered choices. Mean confidence across v2/v3 cannot be compared as quality because their choice sets differ. Calibrate any future numeric gate against observed task outcomes, including corrections and retries, before letting it change Auto. V3 adds one small question to the same Jev call, not another network call or full prompt logging.
